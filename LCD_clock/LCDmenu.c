...
int main(void)
{
	...
	while(1)
	{
		...
		lcd_print_uint(day_t);
		putme(':');
		lcd_print_uint(hour_t);
		putme(':');
		lcd_print_uint(min_t);
		putme(':');
		lcd_print_uint(sec_t);
				...
				if(min_t==59)
 				{
					...
					if(hour_t==24)
					{
						...
						if((hour_t==hora1)||(hora2==hour_t))
						{/*mover huevos xor 2 angles*/}
						hour_t+=1;
					}
				}
				else{
